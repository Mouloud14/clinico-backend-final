...
export const addPrescriptionToPatient = catchAsyncErrors(async (req, res, next) => { // <<< AJOUTER catchAsyncErrors et next
  try {
    const patient = await Patient.findOne({
      _id: req.params.id,
      doctor: req.user._id
    ...
    patient.prescriptions.push(req.body); // <<< ERREUR POTENTIELLE: req.body peut contenir trop de choses
    await patient.save();
