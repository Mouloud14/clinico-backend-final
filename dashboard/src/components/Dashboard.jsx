    ...
    const fetchData = async () => {
      try {
        ...
        const appointmentsResponse = await axios.get(
          `http://localhost:4000/api/v1/patient/by-date?date=${today.toISOString()}`,
          { withCredentials: true }
        );
    ...
    <section className="dashboard page">
      <div className="banner">
        ...
        <div className="thirdBox" onClick={() => navigate("/calendar")} style={{ cursor: "pointer" }}>
          <p>Nombre total de rendez-vous programmés aujourd'hui :</p>
          <h3>{patientsToday}</h3>
          <h5>voir plus...</h5>
        </div>

        {/* Nouvelle quatrième boîte ajoutée ici */}
        <div className="fourthBox" style={{ cursor: "pointer" }}>
